...
5. Add the matplotlib path alongside the SmartPM Playwright capture. Both paths stay available; the colleague picks via command argument. Matplotlib styling will need iteration to look right against live data — Playwright stays as the working fallback through that iteration period. Removal happens in a later branch once the matplotlib output is dialed in and the team is comfortable.
...
FIGSIZE   = (8, 3)      # inches; wide-and-short aspect that reads well in the email column
DPI       = 144         # crisp on Outlook desktop; ~1152×432 px output
...
def render_summary_report(data, output_path):
    ...
    Uses figsize=(8, 9) — three rows of roughly 8×3 each, stacked. Explicit
    height_ratios=[1, 2, 2] so the cards row is shorter than the curve and table.
    Output filename stays smartpm-summary-report.png so generate_email_preview_html.py
    and generate_email_eml.py don't change.
...
### Live-data iteration loop

The implementation plan will render each chart against a real recent project's MCP data (e.g. W1177 Lubumbashi's most recent update) and present the PNGs for review. Expect back-and-forth on styling — fonts, gridlines, axis labels, color choices, marker sizes, table cell formatting — until each chart reads right at email-column size (8"×3"). Each tweak is a small edit to one chart function in `charts.py`; the goldens get rebaselined once Camron approves the look. Playwright stays in place through this iteration so weekly emails can still ship from the legacy path if a matplotlib chart isn't ready.
...
3. **Do not delete the Playwright capture path.** `references/smartpm/capture-smartpm.js`, `env-loader.js`, `smartpm-client.js`, the three spec files, and `take-screenshots.bat` all stay in place and keep working. They are the safety net while matplotlib styling is being dialed in. A later branch removes them once the team is happy.
4. Rewrite `phases/screenshots.md` so it documents both paths. The phase routes by argument:
   - `/schedule-update screenshots` (no arg) → matplotlib path (the new way)
   - `/schedule-update screenshots --legacy` → existing Playwright path, unchanged
   Both write into the same `{dated_folder}/screenshots/` with the same filenames. The downstream phases (`email`, `report`, `draft`) can't tell which path produced the PNGs, so the colleague can mix and match per week (e.g. matplotlib for the 7 trends, `--legacy` for the Summary Report if its composite still needs work).

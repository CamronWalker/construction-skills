// 15-high-total-float.js — High Total Float Activities Over Time.
...
// chart 15 differs only in title, marker fill color (red), and Y-axis
// formatter (percent).

import { renderTrendLine } from './13-missing-logic.js';
import { HTML_CARD_W, HTML_CARD_H } from './svg-lib.js';

const MARKER_FILL = '#b00020';

/**
 * @typedef {Array<{ dataDate: string, value: number }>} HighTotalFloatPayload
 */
...
export const META = {
  ...
  title:     'High Total Float Activities Over Time',
};

/**
 * @param {HighTotalFloatPayload | { trend?: HighTotalFloatPayload }} payload
 * @returns {import('./svg-lib.js').RenderResult}
 */
export function renderHighTotalFloat(payload) {
  let rows;
  if (Array.isArray(payload)) {
    ...
    if (envelope === undefined || envelope === null) {
      ...
      throw new TypeError('expected HighTotalFloatPayload (array) or { trend: array }');
    }
  } else {
    throw new TypeError('expected HighTotalFloatPayload (array) or { trend: array }');
  }

...
0. **Supabase project bindings (`get_project`)** — the new first step. Before touching last-week artifacts, call **`get_project(job_number=<job_number>)`** and map the row via `project_context_db_mapping.project_row_to_context(row)`. This gives you `ctx` (the project **bindings**: `project_name`, SmartPM URLs + project name, Procore ids). On a `get_project` miss, lazy-migrate the legacy `project-context.html` (parse → `upsert_project` → `append_project_log` per entry → `retire_context_html`); if there's no HTML, stop and tell the user to run **schedule-project-init**. Note: bindings are NOT a `prev_draft` — they don't carry recipients / signer / graph_order / the four lists. Those still come from steps 1–5 below (or, on week-1, from the conversational Q&A in `report.md`). This step only ensures `ctx` is populated from the database, replacing the retired `load_project_context(schedules_root)` HTML read.

1. **`{prev_date}-email.json`** — the cloud-finalized v2 dict. Load with `email_draft_io.load_draft(path)`. Primary path for `prev_draft`; everything else below is a fallback.

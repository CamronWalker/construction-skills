...
def find_sc_milestone(tasks):
    ...
    for t in tasks:
        if t.get('task_type', '') in exc: continue
        name = t.get('task_name', '')
        if 'Substantial Completion' in name and 'Turnover to Owner' in name:
            if t.get('status_code', '') != 'TK_Complete':
                return t['task_id']

    # Priority 2: Exact match "Substantial Completion" milestone
    for t in tasks:
        if t.get('task_type', '') in exc: continue
    ...
    for t in tasks:
        if t.get('task_type', '') in exc: continue
        if t.get('task_type', '') not in ('TT_FinMile', 'TT_Mile'): continue
...
After SC filtering, rebuild the relationship set to only include relationships between in-scope tasks:
...
inc_rels = [p for p in preds if p['task_id'] in inc_ids and p['pred_task_id'] in inc_ids]
activities = [t for t in incomplete if t.get('task_type', '') not in milestone_types]
```

Report both total schedule size and the filtered scope so the reviewer understands what was analyzed.
...
Mean total float in working days across in-scope activities (not milestones). Scored bidirectionally — both too-low and too-high float indicate problems. Also used to detect "negative float schedules" — see below.
...
**SKIP this metric entirely if the schedule has negative average float.** When a schedule is behind (all float is negative), the CP% metric becomes meaningless — nearly all or no activities appear "critical" depending on the float threshold used.
...
# Build lookup from ALL relationships in the schedule (not filtered)
all_succs = defaultdict(set)
all_preds_map = defaultdict(set)
for p in all_preds:  # Use the full unfiltered TASKPRED table
    all_succs[p.get('pred_task_id', '')].add(p.get('task_id', ''))
    all_preds_map[p.get('task_id', '')].add(p.get('pred_task_id', ''))

open_ids = set()
for t in activities:  # Only check in-scope non-milestone activities
    tid = t['task_id']
...
**SKIP this metric entirely if the schedule has negative average float** (same reasoning as Critical Path %).
...
```python
hard_codes = {
    'CS_MSO', 'CS_MFO', 'CS_MEO',
    'CS_MANDSTART', 'CS_MANDEND', 'CS_MANDFIN'
}

soft_codes = {
    'CS_SNET', 'CS_SNLT', 'CS_FNET', 'CS_FNLT',
    'CS_MSOA', 'CS_MSOB', 'CS_MEOA', 'CS_MEOB'
}

# NOTE: CS_ALAP is intentionally EXCLUDED — it is not a date constraint
all_constraint_codes = hard_codes | soft_codes

constrained = 0
for t in incomplete:  # In-scope incomplete tasks
...
**Why CS_ALAP is excluded:** As Late As Possible is a scheduling directive, not a date constraint. It doesn't lock an activity to a specific date — it just tells the scheduler to delay the activity as long as possible within available float. SmartPM does not count ALAP as a constraint.

**Why hard + soft combined:** With SC scope filtering applied, the constraint count already reflects only the relevant scope. Scoring hard constraints alone on filtered scope produces scores too close to 100% (most projects have very few truly mandatory constraints). The combined approach with proportional scoring matches SmartPM behavior.
...
def get_predecessor_scope(sc_task_id, preds):
    """Walk backward from SC to get all transitive predecessors."""
    succ_to_preds = defaultdict(set)
    for p in preds:
        succ_to_preds[p.get('task_id', '')].add(p.get('pred_task_id', ''))
    visited = set()
...
After computing the score, generate a Markdown report. Save it to the project folder or the user's preferred output location.
...
3. **Call `compute_quality_score()`** with all tasks and all predecessors (pass the full tables — SC filtering is handled internally)
4. **Call `generate_quality_report()`** to produce the Markdown output
5. **Save the .md file** to the project folder
